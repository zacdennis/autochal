...
{    
	...
	#if PRINT_ANALOGS==1
		Serial.print("AN0:");
		Serial.print(read_adc(0)*Gyro_Gain_X); //Reversing the sign. 
		Serial.print(",AN1:");
		Serial.print(read_adc(1)*Gyro_Gain_Y);
		Serial.print(",AN2:");
		Serial.print(read_adc(2)*Gyro_Gain_Z);  
		Serial.print(",AN3:");
		Serial.print(read_adc(3)/GRAVITY);
		Serial.print (",AN4:");
		Serial.print(read_adc(4)/GRAVITY);
		Serial.print (",AN5:");
		Serial.print(read_adc(5)/GRAVITY);
		Serial.print (",");
	#endif
...
//	} else {
		...
		IMU_buffer[1] = 0x02;
                ...
                tempint = read_adc(0)*Gyro_Gain_X*100; //gyroX
                if(tempint<0) tempint+=50000;
                IMU_buffer[2]=tempint&0xff;
		IMU_buffer[3]=(tempint>>8)&0xff;

                tempint = read_adc(1)*Gyro_Gain_Y*100; //gyroY
                if(tempint<0) tempint+=50000;
                IMU_buffer[4]=tempint&0xff;
		IMU_buffer[5]=(tempint>>8)&0xff;

                tempint = read_adc(2)*Gyro_Gain_Z*100; //gyroZ
                if(tempint<0) tempint+=50000;
                IMU_buffer[6]=tempint&0xff;
		IMU_buffer[7]=(tempint>>8)&0xff;

                templong = read_adc(3)/GRAVITY*1000L; //accelX (accel * 100 in +-4g range)
                if(templong<0)
                  templong+=8000;
		...
		IMU_buffer[9]=(templong>>8)&0xff;
                ...
                templong = read_adc(4)/GRAVITY*1000L; //accelY
                if(templong<0)
                  templong+=8000;
		...
		IMU_buffer[13]=(templong>>8)&0xff;
                ...
                templong = read_adc(5)/GRAVITY*1000L; //accelZ
                if(templong<0)
                  templong+=8000;
                //tempint = templong;
                IMU_buffer[16]=templong&0xff;
		IMU_buffer[17]=(templong>>8)&0xff;
                IMU_buffer[18]=(templong>>16)&0xff;
                IMU_buffer[19]=(templong>>24)&0xff;

		tempint=ToDeg(roll)*100;  //Roll (degrees) * 100 in 2 bytes
                if(tempint<0) tempint+=36000;
		IMU_buffer[20]=tempint&0xff;
		IMU_buffer[21]=(tempint>>8)&0xff;
      
		tempint=ToDeg(pitch)*100;   //Pitch (degrees) * 100 in 2 bytes
                if(tempint<0) tempint+=36000;
		IMU_buffer[22]=tempint&0xff;
		IMU_buffer[23]=(tempint>>8)&0xff;
      
		templong=ToDeg(yaw)*100;  //Yaw (degrees) * 100 in 2 bytes
		//if(templong>18000) templong -=36000;
		//if(templong<-18000) templong +=36000;
                if(templong>36000) templong -=36000;
		if(templong<0) templong +=36000;
		tempint = templong;
		IMU_buffer[24]=tempint&0xff;
		IMU_buffer[25]=(tempint>>8)&0xff;
