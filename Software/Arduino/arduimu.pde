...
#define MAGNETIC_DECLINATION -130.0    // corrects magnetic bearing to true north
...
#define GRAVITY 16384  // This equivalent to 1G in the raw data coming from the accelerometer, per diydrones.com/forum/topics/arduimu
#define Accel_Scale(x) x*(GRAVITY/9.81)//Scaling the raw data of the accel to actual acceleration in meters for seconds square
...
void loop() //Main Loop
{
  ...
  if((timeNow-timer)>=20)  // Main loop runs at 50Hz
  {
	...
	cycleCount++;
		...
		switch (cycleCount) {
			case(0):
				//GPS.update(); //comment out GPS functionality
				break;
			...
			case(4):
				...
				break;*/ //comment out GPS functionality
